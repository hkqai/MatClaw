...
**Background**: 
- Structure relaxation tools (`matgl_relax_structure`) use TensorNet models which require the **PYG** (PyTorch Geometric) backend
- Property prediction tools (`matgl_predict_eform`, `matgl_predict_bandgap`) use M3GNet/MEGNet models which require the **DGL** backend
- MatGL can only use **one backend per Python process** and cannot switch backends once initialized

**Problem**: Running `pytest tests/matgl/ -v` will cause tests to fail because:
...
pytest tests/matgl/test_matgl_predict_eform.py -v

# Run band gap prediction tests (DGL backend)
pytest tests/matgl/test_matgl_predict_bandgap.py -v
```
